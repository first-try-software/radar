...
RSpec.describe ApplicationHelper, type: :helper do
  ...
  describe '#breadcrumb_from_ref' do
    it 'returns nil for blank ref' do
      expect(helper.breadcrumb_from_ref(nil)).to be_nil
      expect(helper.breadcrumb_from_ref('')).to be_nil
    end

    it 'returns dashboard breadcrumb for dashboard ref' do
      result = helper.breadcrumb_from_ref('dashboard')

      expect(result[:name]).to eq('Dashboard')
      expect(result[:path]).to eq('/')
    end

    it 'returns team breadcrumb for team ref' do
      team = TeamRecord.create!(name: 'Platform Team')

      result = helper.breadcrumb_from_ref("team:#{team.id}")

      expect(result[:name]).to eq('Platform Team')
      expect(result[:path]).to eq("/teams/#{team.id}")
    end

    it 'returns nil for team ref with non-existent team' do
      result = helper.breadcrumb_from_ref('team:999999')

      expect(result).to be_nil
    end

    it 'returns initiative breadcrumb for initiative ref' do
      initiative = InitiativeRecord.create!(name: 'Launch 2025')

      result = helper.breadcrumb_from_ref("initiative:#{initiative.id}")

      expect(result[:name]).to eq('Launch 2025')
      expect(result[:path]).to eq("/initiatives/#{initiative.id}")
    end

    it 'returns nil for initiative ref with non-existent initiative' do
      result = helper.breadcrumb_from_ref('initiative:999999')

      expect(result).to be_nil
    end

    it 'returns project breadcrumb for project ref' do
      project = ProjectRecord.create!(name: 'Feature A')

      result = helper.breadcrumb_from_ref("project:#{project.id}")

      expect(result[:name]).to eq('Feature A')
      expect(result[:path]).to eq("/projects/#{project.id}")
    end

    it 'returns nil for project ref with non-existent project' do
      result = helper.breadcrumb_from_ref('project:999999')

      expect(result).to be_nil
    end

    it 'returns nil for unrecognized ref format' do
      result = helper.breadcrumb_from_ref('unknown:123')
