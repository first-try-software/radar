...
class InitiativeTrendService
  ...
  attr_reader :initiative, :health_update_repository

  def projects
    @projects ||= initiative.leaf_projects
  end
  ...
  def calculate_base_confidence
    ...
    # Max std_dev for health scores (-1 to 1) is about 1.0
    # Scale: std_dev 0 = 100, std_dev 0.5 = 50, std_dev 1.0 = 0
  ...
  def most_recent_update_date
    @most_recent_update_date ||= projects.map do |project|
      latest = health_update_repository.latest_for_project(project.id)
      latest&.date&.to_date
    end.compact.max
  end
